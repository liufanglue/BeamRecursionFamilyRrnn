...
class classifier_framework(nn.Module):
    def __init__(self, data, config):
        ...
        if "no_embedding" in self.config and self.config["no_embedding"]:
            if "compress_input" in self.config and self.config["compress_input"]:
                self.embd_dim = 10
            else:
                self.embd_dim = 1
        else:
        ...
        if self.config["classifier_mlp"]:
            ...
            self.prediction_linear = nn.Linear(config["hidden_size"], self.classes_num)
    ...
    def embed(self, sequence_idx, input_mask):
        ...
        if "no_embedding" in self.config and self.config["no_embedding"]:
            ...
            if "compress_input" in self.config and self.config["compress_input"]:
                if S % self.embd_dim != 0:
                    PAD = T.zeros(N, self.embd_dim * math.ceil(S / self.embd_dim) - S).float().to(sequence.device)
                    sequence = T.cat([sequence, PAD], dim=-1)
                    input_mask = T.cat([input_mask, PAD], dim=-1)
                    S = math.ceil(S / self.embd_dim)
                    sequence = sequence.view(N, S, self.embd_dim)
                    input_mask = input_mask.view(N, S, self.embd_dim)[..., 0]
            else:
    ...
    def forward(self, batch):
        ...
        if parse_trees is not None:
            sequence_dict = self.encoder(sequence.detach(), input_mask, parse_trees)
        else:
            sequence_dict = self.encoder(sequence.detach(), input_mask)
